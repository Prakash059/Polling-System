<?php
$link = mysql_connect('localhost', 'root', 'username') or die(mysql_error());
...
//clubs
// retrieving positions sql query
$clubs=mysql_query("SELECT * FROM tbclubs")
or die("There are no records to display ... \n" . mysql_error()); 
?>
<?php
    // retrieval sql query
// check if Submit is set in POST
 ...
 {
 // get position value
 $club = addslashes( $_POST['club'] ); //prevents types of SQL injection
...
<title>Online Voting System for Club Activities:Voting Page</title>
...
xmlhttp.open("GET","vote.php?club="+String,true);
xmlhttp.send();
}
...
<tr>
    ...
    ?>
	<td>Choose Club</td>
    <td><SELECT NAME="club" id="club" onclick="getClub(this.value)">
    <OPTION VALUE="select">select
	    <?php 
    //loop through all table rows
    while ($row=mysql_fetch_array($clubs)){
    echo "<OPTION VALUE=$row[clubname]>$row[clubname]"; 
    //mysql_free_result($positions_retrieved);
    //mysql_close($link);
    }
    ?>
